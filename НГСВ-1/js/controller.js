...
class SeparatorController {
  ...
  bindControls() {
    ...
    if (waterLevelSlider && waterLevelValDisplay && waterLevelOutput) {
      waterLevelSlider.addEventListener('input', () => {
        ...
        // Обновляем окно прогноза по водяному клапану
    ...
    if (oilLevelSlider && oilLevelValDisplay && oilLevelOutput) {
      oilLevelSlider.addEventListener('input', () => {
        ...
        // Обновляем окно прогноза по нефтяному клапану
   ...
   */
  onStateChange(state) {
    // Recommendations panel removed — nothing to update here
  }
  ...
  updateUI() {
    ...
    // Инициализация окон прогнозов
    if (this.view.updateWaterForecast && this.model.getValveForecast) {
      this.view.updateWaterForecast(this.model.getValveForecast('water', state.waterK));
    }
    if (this.view.updateOilForecast && this.model.getValveForecast) {
      this.view.updateOilForecast(this.model.getValveForecast('oil', state.oilU));
    }
