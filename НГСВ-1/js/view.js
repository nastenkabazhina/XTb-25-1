...
class SeparatorView {
  ...
  getElements() {
    return {
      ...
      // Forecasts
      waterForecastBadge: document.getElementById('waterForecastBadge'),
      waterForecastTitle: document.getElementById('waterForecastTitle'),
      waterForecastList: document.getElementById('waterForecastList'),
      oilForecastBadge: document.getElementById('oilForecastBadge'),
      oilForecastTitle: document.getElementById('oilForecastTitle'),
      oilForecastList: document.getElementById('oilForecastList'),
  ...
  /**
   * Обновление прогноза по водяному клапану
   */
  updateWaterForecast(forecast) {
    const badge = this.elements.waterForecastBadge;
    const title = this.elements.waterForecastTitle;
    const list = this.elements.waterForecastList;
    if (!title || !list || !badge) return;

    badge.className = `status-badge ${forecast.status}`;
    badge.textContent = forecast.status === 'normal' ? 'НОРМА' : forecast.status === 'warning' ? 'ВНИМАНИЕ' : 'АВАРИЯ';
    title.textContent = forecast.title || 'Прогноз по водяному клапану';
  ...
  /**
   * Обновление прогноза по нефтяному клапану
   */
  updateOilForecast(forecast) {
    const badge = this.elements.oilForecastBadge;
    const title = this.elements.oilForecastTitle;
    const list = this.elements.oilForecastList;
    if (!title || !list || !badge) return;

    badge.className = `status-badge ${forecast.status}`;
    badge.textContent = forecast.status === 'normal' ? 'НОРМА' : forecast.status === 'warning' ? 'ВНИМАНИЕ' : 'АВАРИЯ';
    title.textContent = forecast.title || 'Прогноз по нефтяному клапану';
