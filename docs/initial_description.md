...
Стиль должен быть в светло голубом и белом цветах. При нажатии на вкладку «Регулятор давления» появляется шкала открытия клапана с ползунком, диапазон которой от 0 до 1, где нижняя граница (0) — клапан закрыт, а верхняя граница (1) — клапан открыт полностью; оптимальное значение (0.5) — клапан открыт наполовину. Также есть шкала расхода жидкости с ползунком (диапазон от 100 до 8000 м³/сут). Расход можно регулировать насосом — значение синхронизируется со слайдером. 
Должно быть большое окно, где будет показано давление в сепараторе (в МПа). Давление рассчитывается по формуле:

P = 20.0 - 19.9 × sqrt((K - 0.1) / (0.9 × (1 + Q/8000)))

где K — степень открытия водяного клапана (waterK), Q — расход эмульсии (inletFlow, м³/сут). Если давление превышает 10 МПа (10 000 кПа), появляется красное мигающее окно с рекомендацией уменьшить напор насоса; если давление ниже 1 МПа (1000 кПа), — появится рекомендация увеличить напор насоса. Когда значение приходит в норму, окно скрывается.
